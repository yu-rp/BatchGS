...
def map_gaussian_to_intersects(
    num_points: int,
    num_intersects: Float[Tensor, "batch"],
    xys: Float[Tensor, "batch n 2"],
    depths: Float[Tensor, "batch n 1"],
    radii: Float[Tensor, "batch n 1"],
    cum_tiles_hit: Float[Tensor, "batch n"],
    tile_bounds: Tuple[int, int, int],
) -> Tuple[Float[Tensor, "batch cum_tiles_hit 1"], Float[Tensor, "batch cum_tiles_hit 1"]]:
...
def get_tile_bin_edges(
    ...
    """
    batch_size = isect_ids_sorted.shape[0]
    return _C.get_tile_bin_edges(
        num_intersects, isect_ids_sorted.contiguous(), tile_bounds, batch_size
    )
...
def compute_cumulative_intersects(
    ...
    """
    cum_tiles_hit = torch.cumsum(num_tiles_hit, dim=1, dtype=torch.int32)
...
def bin_and_sort_gaussians(
    ...
    # print(isect_ids.shape, gaussian_ids.shape)
    isect_ids_sorted, sorted_indices = torch.sort(isect_ids, dim = 1)
    gaussian_ids_sorted = torch.gather(gaussian_ids, 1, sorted_indices)
