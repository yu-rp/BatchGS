...
class _RasterizeGaussians(Function):
    ...
    def forward(
        ...
        depths: Float[Tensor, "*batch n"],
        radii: Float[Tensor, "*batch n"],
        conics: Float[Tensor, "*batch n 3"],
        num_tiles_hit: Int[Tensor, "*batch n"],
        colors: Float[Tensor, "*batch n channels"],
        opacity: Float[Tensor, "*batch n 1"],
        img_height: int,
        img_width: int,
        background: Optional[Float[Tensor, "*batch channels"]] = None,
        return_alpha: Optional[bool] = False,
    ) -> Tensor:
        num_points = xys.size(1)
        ...
        num_intersects, cum_tiles_hit = compute_cumulative_intersects(num_tiles_hit) # b; b,n

        # print("num intersects", num_intersects)

        t1 = time.time()
        (
            isect_ids_unsorted, # b,n
            gaussian_ids_unsorted, # b,n
            isect_ids_sorted, #b,n
            gaussian_ids_sorted, #b,n
            tile_bins, #b,16x16
        ...
        out_img, final_Ts, final_idx = rasterize_fn(
            ...
            background,
            batch_size,
            int(num_intersects.max().item()),
            num_points
        )
        t3 = time.time()
        print("bin and sort time",t2-t1,"rasterize time",t3-t2)
