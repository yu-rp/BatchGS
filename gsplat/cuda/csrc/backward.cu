...
// Each thread handles the backward computation for one pixel
// <<<tile_b.x, tile_b.y, batch_size; 16,16,1>>>
// tile_bounds: (tile_b.x, tile_b.y, 1)
// img_size: (img_w, img_h, 1)
// gaussian_ids_sorted: (b, max_num_intersects)
// tile_bins: (b, 16x16)
// xys: (b, n, 2)
// conics: (b, n, 3)
// rgbs: (b, n, 3)
...
__global__ void rasterize_backward_kernel(
    ...
    const unsigned pix_id_unsigned = min(i * img_size.x + j, img_size.x * img_size.y - 1);
    ...
    // this is the T AFTER the last gaussian in this pixel
    const unsigned output_idx_stride = batch_idx * img_size.x * img_size.y;
    const unsigned output_idx = pix_id_unsigned + output_idx_stride;
    float T_final = final_Ts[output_idx];
    float T = T_final;
    // the contribution from gaussians behind the current one
    float3 buffer = {0.f, 0.f, 0.f};
    // index of last gaussian to contribute to this pixel
    const int bin_final = inside? final_index[output_idx] : 0;

    // have all threads in tile process the same gaussians in batches
    // first collect gaussians between range.x and range.y in batches
    // which gaussians to look through in this tile
    const unsigned tile_bins_idx = batch_idx * tile_size.y * tile_size.x + tile_id;
    const int2 range = tile_bins[tile_bins_idx];
    ...
    // df/d_out for this pixel
    const float3 v_out = v_output[output_idx];
    const float v_out_alpha = v_output_alpha[output_idx];
    ...
    for (int b = 0; b < num_batches; ++b) {
        ...
        if (idx >= range.x) {
            ...
            rgbs_batch[tr] = rgbs[input_index];
        ...
        for (int t = max(0,batch_end - warp_bin_final); t < batch_size; ++t) {
            ...
            if(valid){
                ...
                v_alpha += T_final * ra * v_out_alpha;
                // contribution from background pixel
                v_alpha += -T_final * ra * background[batch_idx].x * v_out.x;
                v_alpha += -T_final * ra * background[batch_idx].y * v_out.y;
                v_alpha += -T_final * ra * background[batch_idx].z * v_out.z;
                // update the running sum
                buffer.x += rgb.x * fac;
                buffer.y += rgb.y * fac;
                buffer.z += rgb.z * fac;
            ...
            if (warp.thread_rank() == 0) {
                ...
                atomicAdd(v_rgb_ptr +  3 * batch_idx * num_points +  3*g + 0, v_rgb_local.x);
                atomicAdd(v_rgb_ptr +  3 * batch_idx * num_points +  3*g + 1, v_rgb_local.y);
                atomicAdd(v_rgb_ptr +  3 * batch_idx * num_points +  3*g + 2, v_rgb_local.z);
                
                float* v_conic_ptr = (float*)(v_conic);
                atomicAdd(v_conic_ptr +  3 * batch_idx * num_points +  3*g + 0, v_conic_local.x);
                atomicAdd(v_conic_ptr +  3 * batch_idx * num_points +  3*g + 1, v_conic_local.y);
                atomicAdd(v_conic_ptr +  3 * batch_idx * num_points +  3*g + 2, v_conic_local.z);
                
                float* v_xy_ptr = (float*)(v_xy);
                atomicAdd(v_xy_ptr +  2 * batch_idx * num_points +  2*g + 0, v_xy_local.x);
                atomicAdd(v_xy_ptr +  2 * batch_idx * num_points +  2*g + 1, v_xy_local.y);
                
                atomicAdd(v_opacity + batch_idx * num_points +  g, v_opacity_local);
            }
            unsigned input_index_temp = batch_idx * num_points + id_batch[t];
        }
    }
}
