...
project_gaussians_forward_tensor(
    ...
    // printf("project_gaussians_forward_tensor\n");
    dim3 img_size_dim3;
...
std::tuple<torch::Tensor, torch::Tensor> map_gaussian_to_intersects_tensor(
    ...
    int max_num_intersects = num_intersects.max().item<int>();
    // printf("max_num_intersects: %d\n", max_num_intersects);

    torch::Tensor gaussian_ids_unsorted =
        torch::full({batch_size, max_num_intersects}, std::numeric_limits<int32_t>::max(), xys.options().dtype(torch::kInt32));
        // torch::zeros({batch_size, max_num_intersects}, xys.options().dtype(torch::kInt32));
    torch::Tensor isect_ids_unsorted =
        torch::full({batch_size, max_num_intersects}, std::numeric_limits<int64_t>::max(), xys.options().dtype(torch::kInt64));
        // torch::zeros({batch_size, max_num_intersects}, xys.options().dtype(torch::kInt64));

    dim3 grid_Shape;
    grid_Shape.x = batch_size;
    grid_Shape.y = (num_points + N_THREADS - 1) / N_THREADS;
    grid_Shape.z = 1;

    map_gaussian_to_intersects<<<grid_Shape,N_THREADS>>>(
        num_points,
...
torch::Tensor get_tile_bin_edges_tensor(
    const torch::Tensor &num_intersects, const torch::Tensor &isect_ids_sorted, 
    const std::tuple<int, int, int> tile_bounds, const int batch_size
) {
    CHECK_INPUT(isect_ids_sorted);
    int num_tiles = std::get<0>(tile_bounds) * std::get<1>(tile_bounds);
    torch::Tensor tile_bins = torch::zeros(
        {batch_size, num_tiles, 2}, isect_ids_sorted.options().dtype(torch::kInt32)
    );
    int max_num_intersects = num_intersects.max().item<int>();

    dim3 grid_Shape;
    grid_Shape.x = batch_size;
    grid_Shape.y = (max_num_intersects + N_THREADS - 1) / N_THREADS;
    grid_Shape.z = 1;

    get_tile_bin_edges<<<grid_Shape,N_THREADS>>>(
...
rasterize_forward_tensor(
    ...
    dim3 grid_dim3;
    grid_dim3.x = std::get<0>(tile_bounds);
    grid_dim3.y = std::get<1>(tile_bounds);
    grid_dim3.z = batch_size;
    // printf("grid_dim3: %d, %d, %d\n", grid_dim3.x, grid_dim3.y, grid_dim3.z);
    ...
    const int channels = colors.size(2);
    ...
    rasterize_forward<<<grid_dim3, block_dim3>>>(
        tile_bounds_dim3,
        img_size_dim3,
        block_dim3,
        max_num_intersects,
...
std::
    ...
    rasterize_backward_tensor(
        ...
        const unsigned max_num_intersects,
        const torch::Tensor &gaussians_ids_sorted,
        const torch::Tensor &tile_bins,
        const torch::Tensor &xys,
        const torch::Tensor &conics,
    ...
    if (xys.ndimension() != 3 || xys.size(2) != 2) {
        AT_ERROR("xys must have dimensions (batch, num_points, 2)");
    }

    if (colors.ndimension() !=  3|| colors.size(2) != 3) {
        AT_ERROR("colors must have 3 dimensions");
    }

    const int num_points = xys.size(1);
    // printf("num_points: %d\n", num_points);
    // printf("img_width: %d\n", img_width);
    // printf("img_height: %d\n", img_height);
    const dim3 tile_bounds = {
    ...
    torch::Tensor v_xy = torch::zeros({batch_size, num_points, 2}, xys.options());
    torch::Tensor v_conic = torch::zeros({batch_size, num_points, 3}, xys.options());
    torch::Tensor v_colors =
        torch::zeros({batch_size, num_points, channels}, xys.options());
    torch::Tensor v_opacity = torch::zeros({batch_size, num_points, 1}, xys.options());
    
    rasterize_backward_kernel<<<grid_dim3, block>>>(
        tile_bounds,
        img_size,
        block,
        max_num_intersects,
