...
__global__ void map_gaussian_to_intersects(
    ...
    const dim3 tile_bounds,
    const int max_num_intersects,
    int64_t* __restrict__ isect_ids,
    int32_t* __restrict__ gaussian_ids
) {
    unsigned batch_idx = blockIdx.x; 
    unsigned idx = blockIdx.y * blockDim.x + threadIdx.x; 
    unsigned input_idx = batch_idx * num_points + idx; 
    // unsigned input_idx = idx;

    if (idx >= num_points)
        return;
    if (radii[input_idx] <= 0)
        return;
    // get the tile bbox for gaussian
    uint2 tile_min, tile_max;
    float2 center = xys[input_idx];
    get_tile_bbox(center, radii[input_idx], tile_bounds, tile_min, tile_max);
    // printf("point %d, %d radius, min %d %d, max %d %d\n", idx, radii[idx],
    // tile_min.x, tile_min.y, tile_max.x, tile_max.y);

    // update the intersection info for all tiles this gaussian hits
    int32_t cur_idx = (idx == 0) ? 0 : cum_tiles_hit[input_idx - 1];
    // printf("point %d starting at %d\n", idx, cur_idx);
    int64_t depth_id = (int64_t) * (int32_t *)&(depths[input_idx]);
    for (int i = tile_min.y; i < tile_max.y; ++i) {
        for (int j = tile_min.x; j < tile_max.x; ++j) {
            // isect_id is tile ID and depth as int32
            int64_t tile_id = i * tile_bounds.x + j; // tile within image
                // unsigned isect_ids_idx = batch_idx* max_num_intersects + cur_idx;
                // unsigned gaussian_ids_idx = batch_idx * max_num_intersects + cur_idx;
            unsigned output_idx = batch_idx * max_num_intersects + cur_idx;
            isect_ids[output_idx] = (tile_id << 32) | depth_id; // tile | depth id
            gaussian_ids[output_idx] = idx;                     // 3D gaussian id
...
// now have an isect_ids_sorted, which is sorted by tile_id, and need to find the start and end positions for each tile.
// For each element in isection_ids_sorted, I want to create a thread.
// The output is a tile_bins, where each element is an int2, with x as the start position and y as the end position.
// The size of tile_bins is batch_size, num_tiles.
__global__ void get_tile_bin_edges(
    ...
    unsigned batch_idx = blockIdx.x;
    unsigned idx = blockIdx.y * blockDim.x + threadIdx.x;
    unsigned input_idx = batch_idx * max_num_intersects + idx;

    int cur_num_intersects = num_intersects[batch_idx];

    if (idx >= cur_num_intersects)
        return;
    // save the indices where the tile_id changes
    int32_t cur_tile_idx = (int32_t)(isect_ids_sorted[input_idx] >> 32);
    unsigned cur_tile_output_idx = batch_idx * num_tiles + cur_tile_idx;
    if (idx == 0 || idx == cur_num_intersects - 1) {
        if (idx == 0)
            tile_bins[cur_tile_output_idx].x = 0;
        if (idx == cur_num_intersects - 1)
            tile_bins[cur_tile_output_idx].y = cur_num_intersects;
        return;
    }
    int32_t prev_tile_idx = (int32_t)(isect_ids_sorted[input_idx - 1] >> 32);
    unsigned prev_tile_output_idx = batch_idx * num_tiles + prev_tile_idx;
    if (prev_tile_idx != cur_tile_idx) {
        tile_bins[prev_tile_output_idx].y = idx;
        tile_bins[cur_tile_output_idx].x = idx;
        return;
    }
}
...
// Render operation is performed for each pixel by allocating a thread,
// Thread space is <<<tile_b.x, tile_b.y, batch_size; 16,16,1>>>
// The inputs involved are: Gaussian index b,n
// tile_bins b,16x16
// xys b,n,2
// conics b,n,3
// colors b,n,3
...
__global__ void rasterize_forward(
    ...
    int32_t tile_id =
        block.group_index().y * tile_bounds.x + block.group_index().x; // 计算的是当前是那个tile的一维索引
    unsigned i =
    ...
    // 跟input 和 output 有关的index
    unsigned batch_idx = block.group_index().z;
    unsigned tile_bins_idx = batch_idx * tile_size.y * tile_size.x + tile_id;


    // have all threads in tile process the same gaussians in batches
    // first collect gaussians between range.x and range.y in batches
    // which gaussians to look through in this tile
    int2 range = tile_bins[tile_bins_idx];
    ...
    for (int b = 0; b < num_batches; ++b) {
        ...
        if (idx < range.y) {
            unsigned gaussian_ids_sorted_idx = batch_idx * max_num_intersects + idx;
            int32_t g_id = gaussian_ids_sorted[gaussian_ids_sorted_idx];
            id_batch[tr] = g_id;
            unsigned input_index = batch_idx * num_points + g_id;
            const float2 xy = xys[input_index];
            const float opac = opacities[input_index];
            xy_opacity_batch[tr] = {xy.x, xy.y, opac};
            conic_batch[tr] = conics[input_index];
        }
        ...
        for (int t = 0; (t < batch_size) && !done; ++t) {
            ...
            const float vis = alpha * T;
            unsigned colors_index = batch_idx * num_points + g;
            const float3 c = colors[colors_index];
    ...
    if (inside) {
        // output index
        unsigned output_idx = batch_idx * img_size.x * img_size.y + pix_id;
        // add background
        final_Ts[output_idx] = T; // transmittance at last gaussian in this pixel
        final_index[output_idx] =
            cur_idx; // index of in bin of last gaussian in this pixel
        float3 final_color;
        final_color.x = pix_out.x + T * background[batch_idx].x;
        final_color.y = pix_out.y + T * background[batch_idx].y;
        final_color.z = pix_out.z + T * background[batch_idx].z;
        out_img[output_idx] = final_color;
    }
}
